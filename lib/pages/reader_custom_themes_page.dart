...
class _ReaderCustomThemesPageState extends State<ReaderCustomThemesPage> {
  ...
  late final List<ReaderCustomTheme> _themes = [...widget.initialThemes];
  late String? _selectedThemeId = _themes.any(
    (theme) => theme.id == widget.initialSelectedThemeId,
  )
  ...
  Future<void> _persist() {
    ...
    _persistQueue = _persistQueue.then((_) => widget.store.saveAll(snapshot));
    ...
    setState(() {
      ...
      if (_selectedThemeId == theme.id) {
        if (_themes.isEmpty) {
          ...
          _selectedThemeId = _themes[index.clamp(0, _themes.length - 1)].id;
        }
      }
    });
    await _persist();
  ...
  void _reorder(int oldIndex, int newIndex) {
    setState(() {
      final theme = _themes.removeAt(oldIndex);
      _themes.insert(newIndex, theme);
    });
    unawaited(_persist());
  }
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(context.l10n.readerCustomThemesTitle),
          actions: [
            IconButton(
            ...
            children: [
              ...
              Expanded(
                child: _themes.isEmpty
                    ? _EmptyCustomThemes(onAdd: _addTheme)
                    : ReorderableListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Padding(
                            key: ValueKey('custom-theme-list-${theme.id}'),
                            padding: const EdgeInsets.only(bottom: 12),
                            child: _CustomThemeCard(
                              theme: theme,
                              name: _displayName(theme, index),
...
class _CustomThemeCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final palette = ReaderThemes.fromCustomTheme(theme);
    return Material(
      color: Colors.transparent,
      child: InkWell(
        onTap: onTap,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              context.l10n.readerCustomThemeEmptyTitle,
            ...
            Text(
              context.l10n.readerCustomThemeEmptyHint,
            ...
            FilledButton.icon(
              onPressed: onAdd,
              icon: const Icon(Icons.add_rounded),
              label: Text(context.l10n.readerCustomThemeAdd),
            ),
          ],
        ),
      ),
