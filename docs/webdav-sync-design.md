...
| 数据域 | 默认 | 合并方式 | 说明 |
| --- | --- | --- | --- |
| 书源注册列表 | 开 | 源 ID 级 LWW + tombstone | 同步公开清单信息、启用状态和添加时间；不含凭据、令牌或缓存 |
...
远端同步到本机、但本地原文件尚不可用的普通本地书籍，不应以空 `filePath` 强行插入现有 `books` 表。书库聚合层可把 `sync_records` 中的 `remoteOnly` 书籍投影为“未下载”卡片；当用户下载并通过现有导入服务校验入库后，再建立 `book_uid -> books.id` 映射。在线书源书会携带公开书源快照与在线书籍快照，并在 `book_sources` 数据集先物化后，按 `source_id + source_book_id` 直接恢复为 `storage_type = online` 的书架项；设备本地封面路径和章节缓存不会进入元数据同步。
