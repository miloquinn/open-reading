...
| 笔记/高亮 | 开 | 稳定 UUID + LWW + tombstone | 内容、颜色、定位和用户笔记作为一个记录版本 |
...
- 在线书源章节缓存不作为书籍文件同步，避免复制可重新获取或可能受授权限制的内容。
...
- 首版不是端到端加密：WebDAV 服务提供方能读取元数据、笔记和用户选择上传的文件。设置页必须明确披露。
...
3. 点击“测试连接”；测试成功后才能保存并开启自动同步。
4. 选择同步范围。书籍原文件默认关闭，并显示预计上传大小。
...
- 新增同步表和迁移。
- 为书签、笔记、阅读会话建立稳定身份和 tombstone 语义。
- 为书籍建立 `book_uid` 映射。
...
- 同步书架元数据、CanonicalLocator 进度、书签、笔记和阅读会话。
- 设置页入口、状态页、手动同步和前台触发。
...
- 两台新设备依次首次同步。
- 两台设备离线修改不同记录后合并。
- 两台设备离线修改同一笔记，HLC 决胜稳定且重复同步不抖动。
- 新进度比旧进度页码更小但时间更新，仍正确覆盖。
- 一端删除书签/笔记，另一端离线编辑后再上线的决胜行为符合 HLC。
